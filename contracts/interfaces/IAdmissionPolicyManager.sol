...
interface IAdmissionPolicyManager {
    ...
    struct AdmissionPolicy {
        ...
        bool[50] reservedSlots;
    }

    event Deployed(address deployer, address trustedForwarder, address policyManager_);
    event CreateAdmissionPolicy(address user, bytes32 admissionPolicyId, bytes32 policyId, uint256 quorum, uint256 secondsToLive, bytes32 userAdminPolicy);
    event UpdateAdmissionPolicy(address user, bytes32 admissionPolicyId, bytes32 policyId, uint256 quorum, uint256 secondsToLive);
    ...
    function updateAdmissionPolicy(bytes32 admissionPolicyId,bytes32 policyId, uint256 quorum, uint256 secondsToLive) external;
