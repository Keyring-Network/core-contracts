...
contract GroupRegistry is IGroupRegistry, KeyringAccessControl, Initializable {
    ...
    bytes32 public constant ROLE_GROUP_MASTER = keccak256("role group master");
    bytes32 public constant ROLE_DEPENDENCY_MANAGER = keccak256("role dependency_manager");

    Bytes32Set.Set groupSet;
    ...
    modifier onlyGroupMaster {
        _checkRole(
            ROLE_GROUP_MASTER,
            _msgSender(),
            "GroupRegistry:onlyGroupMaster");
        _;
    }

    modifier onlyDependencyManager {
        _checkRole(
            ROLE_DEPENDENCY_MANAGER,
            _msgSender(),
            "GroupRegistry:onlyDependencyManager");
        _;
    }
    ...
    modifier validateInput(string memory description, string memory uri) {
        require(
            ...
            "GroupRegistry:createGroup: description cannot be empty");
        require(
            bytes(uri).length > 0,
            "GroupRegistry:createdGroup: uri cannot be empty");
        _;
    }
        ...
        returns(bytes32 groupId) 
    {
        ...
        groupSet.insert(
            groupId,
            "GroupRegistry:createGroup: generated duplicated id. Try again.");
    ...
    function updateGroup(bytes32 groupId, string memory description, string memory uri) 
        external 
        onlyGroupMaster 
        validateInput(description, uri)
    {
        require(
            groupSet.exists(groupId),
            "GroupRegistry.updateGroup: groupId not found");
        Group storage g = groups[groupId];
        g.description = description;
        g.uri = uri;
    ...
    function removeGroup(bytes32 groupId)
        external 
        onlyGroupMaster 
    {
        require(
        ...
        groupSet.remove(
            groupId,
            "GroupRegistry.removeGroup: groupId not found");
    ...
    function addDependency(bytes32 groupId, bytes32 policyId) external onlyDependencyManager {
        require(
            isGroup(groupId),
            "GroupRegistry:addDependency: groupId not found");
        Group storage g = groups[groupId];
        g.policyDependencySet.insert(
            policyId,
            "GroupRegistry:addDependency");
    ...
    function removeDependency(bytes32 groupId, bytes32 policyId) external onlyDependencyManager {
        ...
        Group storage g = groups[groupId];
        g.policyDependencySet.remove(
            policyId,
            "GroupRegistry:removeDependency");
