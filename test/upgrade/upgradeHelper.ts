...
describe("Upgrade Helper", () => {
    ...
    it("should upgrade KeyringCredentials", async () => {
        let proxyAddress = credentials.address;
    ...
    it("should upgrade KeyringZkVerifier", async () => {
        ...
        const constructorArgs = [identityConstructionProofVerifier.address, identityMembershipProofVerifier.address, authorizationProofVerifier.address];
        await upgradeContract(ethers, "KeyringZkVerifier", proxyAddress, constructorArgs);
        expect(await keyringZkVerifier.IDENTITY_CONSTRUCTION_PROOF_VERIFIER()).to.equal(identityConstructionProofVerifier.address);
        expect(await keyringZkVerifier.IDENTITY_MEMBERSHIP_PROOF_VERIFIER()).to.equal(identityMembershipProofVerifier.address);
